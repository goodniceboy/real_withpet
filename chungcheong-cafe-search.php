...
if (!empty($selectedArea) && $selectedArea != '지역구를 선택해주세요' || !empty($addressInput)) {
  $sql = "SELECT `시설명`, `카테고리1`, `카테고리2`, `카테고리3`, `시도 명칭`, `시군구 명칭`, `법정읍면동명칭`, `리 명칭`, `번지`, `도로명 이름`, `건물 번호`, `우편번호`, `도로명주소`, `지번주소`, `전화번호`, `홈페이지`, `휴무일`, `운영시간`, `주차 가능여부`, `입장(이용료)가격 정보`, `반려동물 전용 정보`, `입장 가능 동물 크기`, `반려동물 제한사항`, `장소(실내) 여부`, `장소(실외)여부`, `기본 정보_장소설명`, `애견 동반 추가 요금`, `최종작성일` FROM wpdata WHERE (`시도 명칭` LIKE '%충청%' OR `시도 명칭` = '세종특별자치시') AND `카테고리2` LIKE '%카페%'";
}
